...
import { Listing, ListingWrite, Price, PriceRead } from "@/types.generated";
...
function tableRowToListingPrice(row: ListingPriceTableRow): PriceRead {
  return {
    price_eur: row.price,
    created_date: row.created_date.toISOString(),
  ...
  return {
    ...
    async getListingPrices(listingId: number): Promise<Price[]> {
      ...
      if (result.rows.length == 0) {
        throw new EntityNotFound(
          `Could not find listing prices with id: ${listingId}`
        );
      }
    ...
    async insertListingPrice(listing: Listing) {
      const tableRow = listingPriceToTableRow(listing, new Date());
